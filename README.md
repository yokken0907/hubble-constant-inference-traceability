...
Version 1.5.5 remains the immutable historical public release preceding formal integration of the later TDCOSMO second-implementation evidence. This local version 1.7.0 package is final and internally verified; it does not repoint or rewrite the `v1.5.5` tag or release.

The v1.7.0 tag and release must be created without moving or rewriting the preserved v1.6.0 tag. Only identifiers that actually exist are recorded.
...
MANUSCRIPT/                synchronized final DOCX and rendered PDF
...
Use [`CITATION.cff`](CITATION.cff). The public repository URL is recorded there. If a `v1.7.0` release URL or Jxiv metadata is later issued, record only the actual public identifiers. The manuscript should remain the primary scientific source and this repository its associated traceability archive.
