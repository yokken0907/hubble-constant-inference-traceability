...
上記の公開リポジトリURLは確認・記録済みです。現時点では、未確定のJxiv DOI、Jxiv URL、Release URL、永続識別子、論文公開ステータスを記載していません。残る情報が実際に確定した後にのみ [`PUBLICATION_UPDATE_GUIDE.md`](PUBLICATION_UPDATE_GUIDE.md) に従って更新します。
...
[`CITATION.cff`](CITATION.cff)を参照してください。公開リポジトリURLは記録済みであり、Jxiv情報は公開レコードが実際に成立した後にのみ追加します。科学的な主要引用先は論文であり、このリポジトリは対応するトレーサビリティ・アーカイブです。
