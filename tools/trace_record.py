#!/usr/bin/env python3
"""Print a publication statement (Cxxx), numerical result (Nxxx), source (Sxxx), or post-synthesis validation (Vxxx), re-execution evidence (Exxx), or reproduction status (Rxxx)."""
...
REGISTERS = {
    ...
    "V": (ROOT / "PROVENANCE/POST_SYNTHESIS_VALIDATION_REGISTER.tsv", "VALIDATION_ID"),
    "E": (ROOT / "PROVENANCE/REEXECUTION_EVIDENCE_REGISTER.tsv", "EVIDENCE_ID"),
    "R": (ROOT / "PROVENANCE/REPRODUCTION_STATUS.tsv", "REPRODUCTION_ID"),
}
...
def main() -> int:
    ...
    parser.add_argument("identifier", help="Stable identifier such as C026, N001, S001, V001, E001, or R012")
    args = parser.parse_args()

    identifier = args.identifier.strip().upper()
    if len(identifier) < 2 or identifier[0] not in REGISTERS:
        parser.error("identifier must begin with C, N, S, V, E, or R")
