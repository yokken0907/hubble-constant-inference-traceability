#!/usr/bin/env python3
"""Print a publication statement (Cxxx), numerical result (Nxxx), source (Sxxx), or post-synthesis validation (Vxxx)."""
...
def main() -> int:
    ...
    parser.add_argument("identifier", help="Stable identifier such as C026, N001, S001, or V001")
    args = parser.parse_args()

    identifier = args.identifier.strip().upper()
    if len(identifier) < 2 or identifier[0] not in REGISTERS:
        parser.error("identifier must begin with C, N, S, or V")
