#!/usr/bin/env python3
"""Independent publication-package verifier for v1.7.2.
...
def cap001_release_inventory(audit: Audit, final_mode: bool) -> None:
    ...
    required = {
        ...
        "AI_DISCLOSURE.md", "CITATION.cff", "RELEASE_NOTES_v1.7.2.md",
    ...
    if version.is_file():
        audit.require(cap, version.read_text(encoding="utf-8").strip() == "1.7.2", "VERSION_NOT_1.7.2")
    for path in ROOT.rglob("*"):
...
def cap009_official_fetch(audit: Audit) -> None:
    ...
    active_text = "\n".join((ROOT / rel).read_text(encoding="utf-8", errors="ignore") for rel in ("README.md", "README_JA.md", "REPRODUCIBILITY.md", "RELEASE_CHECKLIST.md", "RELEASE_NOTES_v1.7.1.md", "RELEASE_NOTES_v1.7.2.md", "PROVENANCE/REEXECUTION_EVIDENCE_REGISTER.tsv") if (ROOT / rel).is_file())
...
def cap011_active_state(audit: Audit) -> None:
    ...
    reader_docs = (
        ...
        "RELEASE_CHECKLIST.md", "RELEASE_NOTES_v1.7.2.md",
    )
...
def main() -> int:
    ...
    mode = "FINAL_PACKAGE" if args.final_package else "V1_7_2_METADATA_CHECKPOINT"
    ...
    if args.final_package and integrity:
        ...
        print("V1_7_2_METADATA_PACKAGE_READY = PASS")
    else:
        print("V1_7_2_METADATA_PACKAGE_READY = HOLD_DOCUMENT_SYNC_AND_FINAL_ASSEMBLY")
    if not integrity:
