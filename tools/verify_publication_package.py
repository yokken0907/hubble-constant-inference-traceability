#!/usr/bin/env python3
"""Independent publication-package verifier for v1.7.3.
...
def cap001_release_inventory(audit: Audit, final_mode: bool) -> None:
    cap = "CAP001"
    required = {
        "VERSION", "README.md", "README_JA.md", "README.txt", "REPRODUCIBILITY.md", "RELEASE_CHECKLIST.md",
        "RELEASE_STATUS.md", "PUBLICATION_UPDATE_GUIDE.md", "REPOSITORY_CONTENT_POLICY.md",
        "AI_DISCLOSURE.md", "CITATION.cff", "RELEASE_NOTES_v1.7.2.md", "RELEASE_NOTES_v1.7.3.md",
        "V1_7_1_TO_V1_7_2_METADATA_CORRECTION_RECORD.md",
        "V1_7_2_TO_V1_7_3_DISCLOSURE_NORMALIZATION_RECORD.md",
    ...
    if version.is_file():
        audit.require(cap, version.read_text(encoding="utf-8").strip() == "1.7.3", "VERSION_NOT_1.7.3")
    for path in ROOT.rglob("*"):
...
def cap009_official_fetch(audit: Audit) -> None:
    ...
    active_text = "\n".join((ROOT / rel).read_text(encoding="utf-8", errors="ignore") for rel in ("README.md", "README_JA.md", "REPRODUCIBILITY.md", "RELEASE_CHECKLIST.md", "RELEASE_NOTES_v1.7.1.md", "RELEASE_NOTES_v1.7.2.md", "RELEASE_NOTES_v1.7.3.md", "PROVENANCE/REEXECUTION_EVIDENCE_REGISTER.tsv") if (ROOT / rel).is_file())
...
def cap011_active_state(audit: Audit) -> None:
    ...
    reader_docs = (
        "README.md", "README_JA.md", "README.txt", "RELEASE_STATUS.md",
        "PUBLICATION_UPDATE_GUIDE.md", "REPOSITORY_CONTENT_POLICY.md",
        "AI_DISCLOSURE.md", "CITATION.cff", "REPRODUCIBILITY.md",
        "RELEASE_CHECKLIST.md", "RELEASE_NOTES_v1.7.3.md",
    )
    obsolete_current_tokens = (
        "Jxiv", "pre-Jxiv", "NOT_CREATED", "UNTAGGED",
        "REMOTE_TAG_ACTION", "MANUSCRIPT_STATUS",
    )
    metadata_hits: list[str] = []
    for rel in reader_docs:
        path = ROOT / rel
        if not path.is_file():
            audit.fail(cap, f"CURRENT_METADATA_FILE_MISSING:{rel}")
            continue
        text = path.read_text(encoding="utf-8", errors="ignore")
        for token in obsolete_current_tokens:
            if token.lower() in text.lower():
                metadata_hits.append(f"{rel}:{token}")
    for hit in metadata_hits:
        audit.fail(cap, f"OBSOLETE_CURRENT_METADATA:{hit}")

    obsolete_branding = (
        "v1.7.1-AIRR2",
        "An AI-Led Public-Resource Research Report",
        "AI主導・公開資料研究報告",
    )
    ...
    for rel in reader_docs:
        ...
        if not path.is_file():
            continue
        text = path.read_text(encoding="utf-8", errors="ignore")
        for token in obsolete_branding:
            if token.lower() in text.lower():
                branding_hits.append(f"{rel}:{token}")
    for hit in branding_hits:
        audit.fail(cap, f"OBSOLETE_CURRENT_BRANDING:{hit}")

    audit.metrics["ACTIVE_STALE_REFERENCE_HITS"] = len(hits)
    audit.metrics["OBSOLETE_CURRENT_METADATA_HITS"] = len(metadata_hits)
    audit.metrics["OBSOLETE_CURRENT_BRANDING_HITS"] = len(branding_hits)
...
def main() -> int:
    ...
    mode = "FINAL_PACKAGE" if args.final_package else "V1_7_3_METADATA_CHECKPOINT"
    ...
    if args.final_package and integrity:
        ...
        print("V1_7_3_METADATA_PACKAGE_READY = PASS")
    else:
        print("V1_7_3_METADATA_PACKAGE_READY = HOLD_DOCUMENT_SYNC_AND_FINAL_ASSEMBLY")
    if not integrity:
