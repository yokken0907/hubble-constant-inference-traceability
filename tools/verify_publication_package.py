#!/usr/bin/env python3
"""Independent publication-package verifier for v1.7.0.
...
def cap001_release_inventory(audit: Audit, final_mode: bool) -> None:
    ...
    if version.is_file():
        audit.require(cap, version.read_text(encoding="utf-8").strip() == "1.7.0", "VERSION_NOT_1.7.0")
    for path in ROOT.rglob("*"):
...
def cap009_official_fetch(audit: Audit) -> None:
    ...
    active_text = "\n".join((ROOT / rel).read_text(encoding="utf-8", errors="ignore") for rel in ("README.md", "README_JA.md", "REPRODUCIBILITY.md", "RELEASE_CHECKLIST.md", "RELEASE_NOTES_v1.7.0.md", "PROVENANCE/REEXECUTION_EVIDENCE_REGISTER.tsv") if (ROOT / rel).is_file())
